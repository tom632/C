...
--First check the reports table for the crime
SELECT description
FROM crime_scene_reports
WHERE year = '2021' AND month = '7' AND day = '28' AND street = 'Humphrey Street';

--check who the witnesses are
SELECT name, transcript
FROM interviews WHERE year = '2021' AND month = '7' AND day = '28';

--Check how many people are called Eugene
SELECT name
FROM people
WHERE name = 'Eugene';

--Find the names of the 3 witnesses
SELECT name, transcript
FROM interviews
WHERE year = '2021' AND month = '7' AND day = '28' AND transcript LIKE '%bakery%'
ORDER BY name;

--Check the atm records based on the clue from Eugene
SELECT account_number, amount
FROM atm_transactions
WHERE year = '2021' AND month = '7' AND day = '28' AND atm_location = 'Leggett Street'
AND transaction_type = 'withdraw';

--Find which account belong to which person
...
--fidn the information for the airport
SELECT abbreviation, full_name, city
FROM airports
WHERE city = 'Fiftyville';

--find the flights from fiftyville on the 29th
...
--Check the list of passengers in the flights
SELECT passengers.flight_id, people.name, passengers.passport_number, passengers.seat
FROM people
...
--Check possible names of callers
...
--Check who the recived the call
SELECT people.name, phone_calls.duration
FROM people
...
--Ruth gave a clue that the car left within 10 mins of the theft
